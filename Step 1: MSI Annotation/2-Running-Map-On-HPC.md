...
```
You should recieve an email telling you when your job has started and stopped. When you navigate to the designated output folder in your project directory through the SFTP tab, you should find the output files (a .csv and a .html file). The .csv file contains prediction percents, and the html file shows the predicted MSI status of the sample (image shown below)! There will also be an output text file (MAP_output.txt) generated by the batch job that you may look into to debug if any errors pop up.

![Alt text](https://github.com/alfalfacow/Lab-Notebook/blob/main/Images/Map_Result.png)
