...
```
You should recieve an email telling you when your job has started and stopped. When you navigate to the output folder in your project directory through the SFTP tab, you should find the output files (and thus the predicted MSI status of the sample)!!! There will also be an output text file (MAP_output.txt) generated by the batch job that you may look into to debug if any errors pop up.
